...
def _other_reason_expander(data_df: pd.DataFrame) -> None:
    """
    Render an expander: 'Unclassified / Other Reasons Analysis'.
    Shows count, raw FDA text, auto-classification of recurring patterns,
    and a frequency table. Call this immediately after reason_bar().
    """
    import re as _re2

    # Rows the main chart excludes
    excluded_lower = {"", "other", "not specified", "n/a", "unknown"}
    reason_col = data_df["reason"].fillna("").str.strip() if "reason" in data_df.columns else pd.Series(dtype=str)
    unclassified = data_df[reason_col.str.lower().isin(excluded_lower)].copy()
    other_only   = data_df[reason_col.str.lower() == "other"].copy()
    blank_only   = data_df[reason_col == ""].copy()

    total        = len(data_df)
    n_unclassified = len(unclassified)
    n_other        = len(other_only)
    n_blank        = len(blank_only)

    label = f"Unclassified / Other Reasons Analysis  ({n_unclassified:,} records excluded from chart above)"
    with st.expander(label, expanded=False):
        st.markdown(
            f"""
            <div style="background:#FEF3C7;border-left:3px solid #D97706;border-radius:6px;
                 padding:12px 16px;margin-bottom:16px;font-size:0.84rem;color:#78350F">
              <b>{n_unclassified:,} records ({round(n_unclassified/total*100,1) if total else 0}% of dataset)</b>
              were excluded from the chart above because the FDA did not provide a specific
              shortage reason. This group contains:
              <b>{n_blank:,} blank/unspecified</b> and
              <b>{n_other:,} labelled "Other"</b> by the FDA.
            </div>
            """,
        ...
        if n_other == 0:
            st.info("No records with reason='Other' in current snapshot.")
            return

        # ── Pull raw_json for 'Other' records only ────────────────────────────
        raw_snapshot = db.get_latest_snapshot(limit=2000)
        other_raw_rows = [r for r in raw_snapshot if (r.get("reason") or "").strip() == "Other"]

        def _mfr_reason(ri: str) -> str:
            m = _re2.search(r"Shortage per Manufacturer:\s*([^;\.]+)", ri or "", _re2.IGNORECASE)
            return m.group(1).strip() if m else ""

        # Auto-classify via keyword matching on related_info / mfr reason
        PATTERNS = [
            ("Manufacturing Delay",  _re2.compile(r"manufactur\w* delay", _re2.I)),
            ("Quality / GMP Issue",  _re2.compile(r"(quality|GMP|good manufactur)", _re2.I)),
            ("Supply Chain / Raw Material", _re2.compile(r"(supply|raw material|active ingredient|API)", _re2.I)),
            ("Demand Surge",         _re2.compile(r"(demand|surge|increas)", _re2.I)),
            ("Regulatory / FDA Hold",_re2.compile(r"(regulatory|FDA hold|recall|import alert)", _re2.I)),
            ("Shipping / Logistics", _re2.compile(r"(ship|logistic|transit|deliver)", _re2.I)),
            ("Discontinuation",      _re2.compile(r"discontinu", _re2.I)),
        ]

        classified_counts: dict[str, int] = {}
        raw_mfr_reasons: list[str] = []

        for r in other_raw_rows:
            try:
                raw = json.loads(r.get("raw_json") or "{}")
            except Exception:
                raw = {}
            ri    = raw.get("related_info") or ""
            mfr_r = _mfr_reason(ri)
            raw_mfr_reasons.append(mfr_r or "(no manufacturer sub-reason)")
            combined = f"{ri} {mfr_r}"
            matched = False
            for label_name, pat in PATTERNS:
                if pat.search(combined):
                    classified_counts[label_name] = classified_counts.get(label_name, 0) + 1
                    matched = True
                    break
            if not matched:
                classified_counts["Unclassifiable"] = classified_counts.get("Unclassifiable", 0) + 1

        # ── Auto-classified breakdown chart ───────────────────────────────────
        import plotly.graph_objects as _go2

        cls_df = pd.DataFrame(
            sorted(classified_counts.items(), key=lambda x: x[1]),
            columns=["Category", "Count"],
        )
        cls_df["Pct"] = (cls_df["Count"] / n_other * 100).round(1)
        palette = ["#1A56DB","#0694A2","#6C2BD9","#057A55","#C27803","#C81E1E","#64748B","#94A3B8"]
        bar_colors = [palette[i % len(palette)] for i in range(len(cls_df))]

        fig_cls = _go2.Figure(_go2.Bar(
            x=cls_df["Count"], y=cls_df["Category"],
            orientation="h",
            marker=dict(color=bar_colors, line=dict(color="white", width=0.5)),
            text=[f"{c}  ({p}%)" for c, p in zip(cls_df["Count"], cls_df["Pct"])],
            ...
            customdata=cls_df["Pct"],
        ))
        lay = theme.plotly_base(
            title="Auto-Classified Patterns inside 'Other'  (parsed from related_info)",
            margin=dict(t=52, b=90, l=200, r=100),
        )
        lay["xaxis"].update(title="Records", showgrid=True)
        fig_cls.update_layout(**lay)
        plot(fig_cls)

        # ── Raw FDA reason frequency table ────────────────────────────────────
        st.markdown("**Top raw FDA `shortage_reason` values within the excluded group:**")
        raw_freq = (
            pd.Series(raw_mfr_reasons)
            .value_counts()
            .head(20)
            .reset_index()
        )
        raw_freq.columns = ["Raw Reason Text", "Count"]
        raw_freq["% of Other"] = (raw_freq["Count"] / n_other * 100).round(1).astype(str) + "%"
        df_show(raw_freq, height=400)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    page_header(
        "Shortage Cause Analysis",
        "Drill-down into the 'Other' classification — what's really inside it",
    )
    ...
    _T = theme.get()
    _dark = _T.name == "dark"
    _exp_bg      = "#2B2D42"          if _dark else "#EFF6FF"
    _exp_border  = "rgba(255,255,255,0.12)" if _dark else "#BFDBFE"
    _exp_accent  = "#5B8CFF"          if _dark else "#1A56DB"
    _exp_text    = "#ECEFF4"          if _dark else "#1E3A5F"
    _exp_sub     = "#B7C0D1"          if _dark else "#1E3A5F"
    _code_bg     = "#1F2937"          if _dark else "#DBEAFE"
    _code_text   = "#F9FAFB"          if _dark else "#1E3A5F"
    _code_border = "rgba(255,255,255,0.18)" if _dark else "transparent"
    st.markdown(
        f"""
        <div style="background:{_exp_bg};border:1px solid {_exp_border};
             border-left:4px solid {_exp_accent};border-radius:10px;
             padding:18px 22px;margin-bottom:20px">
          <h4 style="margin:0 0 8px;color:{_exp_text};font-size:0.95rem">How shortage reasons are assigned</h4>
          <p style="margin:0 0 6px;font-size:0.87rem;color:{_exp_sub};line-height:1.6">
            The FDA Drug Shortages API returns a single top-level field called
            <code style="background:{_code_bg};color:{_code_text};border:1px solid {_code_border};
                  padding:1px 5px;border-radius:3px">shortage_reason</code>
            for each record. This field is a plain string chosen from a small fixed vocabulary
            (<em>Demand increase for the drug</em>, <em>Shortage of an active ingredient</em>,
            <em>Discontinuation of the manufacture of the drug</em>, etc.).
          </p>
          <p style="margin:0;font-size:0.87rem;color:{_exp_sub};line-height:1.6">
            When neither the FDA nor the manufacturer provides a specific reason,
            <code style="background:{_code_bg};color:{_code_text};border:1px solid {_code_border};
                  padding:1px 5px;border-radius:3px">shortage_reason</code>
            is set to <strong style="color:{_exp_text}">"Other"</strong>. There is no sub-classification in that field.
            However, the free-text
            <code style="background:{_code_bg};color:{_code_text};border:1px solid {_code_border};
                  padding:1px 5px;border-radius:3px">related_info</code>
            field sometimes contains a manufacturer-reported reason in the pattern
            <em>"Shortage per Manufacturer: &lt;text&gt;"</em> — this analysis extracts and surfaces that signal.
          </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # ── Pull "Other" records with raw_json from DB ────────────────────────────
    # We need raw_json for related_info / therapeutic_category / availability
    # which are not stored in the flattened df columns.
    raw_snapshot = db.get_latest_snapshot(limit=2000)
    other_raw = [r for r in raw_snapshot if (r.get("reason") or "").strip() == "Other"]

    total_records  = len(df)
    other_count    = len(other_raw)
    other_pct      = round(other_count / total_records * 100, 1) if total_records else 0

    if other_count == 0:
        st.info("No records classified as 'Other' in the current snapshot.")
        st.stop()

    # ── Parse related_info for manufacturer sub-reasons ───────────────────────
    def _parse_mfr_reason(related_info: str) -> str:
        """Extract 'Shortage per Manufacturer: <X>' from related_info."""
        m = _re.search(r"Shortage per Manufacturer:\s*([^;\.]+)", related_info or "", _re.IGNORECASE)
        return m.group(1).strip() if m else ""

    # Build enriched records list
    enriched = []
    for r in other_raw:
        try:
            raw = json.loads(r.get("raw_json") or "{}")
            ...
            raw = {}
        ri         = raw.get("related_info") or ""
        mfr_reason = _parse_mfr_reason(ri)
        tc_list    = raw.get("therapeutic_category") or []
        enriched.append({
            ...
            "therapeutic_category": ", ".join(tc_list) if tc_list else "",
            "mfr_reason":        mfr_reason,
            "related_info":      ri,
    ...
    # ── KPI cards ─────────────────────────────────────────────────────────────
    section("'Other' at a Glance")
    has_mfr_reason = edf["mfr_reason"].str.strip().ne("").sum()
    no_sub_reason  = other_count - has_mfr_reason
    availability_unavail = (edf["availability"] == "Unavailable").sum()

    st.markdown(
        "\n".join([
            '<div class="other-kpi-grid">',
            kpi_card("'Other' Records",       f"{other_count:,}",
                     f"{other_pct}% of all records",                  "", "#92400E"),
            kpi_card("Manufacturer Sub-Reason", f"{has_mfr_reason:,}",
                     f"{round(has_mfr_reason/other_count*100,1)}% have a stated cause", "", "#1A56DB"),
            kpi_card("No Further Detail",     f"{no_sub_reason:,}",
                     "FDA says 'Other', nothing more",                "", "#64748B"),
            kpi_card("Unavailable",           f"{availability_unavail:,}",
                     "Drug currently unavailable",                    "", "#C81E1E"),
            "</div>",
        ]),
        unsafe_allow_html=True,
    )
    ...
    freq_df.columns = ["Raw Reason Text", "Count"]
    non_placeholder = freq_df[freq_df["Raw Reason Text"] != "Not further specified"]
    if non_placeholder.empty:
        st.info("No manufacturer sub-reasons were extracted from this data set.")
    else:
        freq_df["% of Other"] = (freq_df["Count"] / other_count * 100).round(1).astype(str) + "%"
        df_show(freq_df, height=400)

    # ── Therapeutic category breakdown ────────────────────────────────────────
    section("Therapeutic Category Breakdown")
    tc_rows = []
    for _, row in edf.iterrows():
        if row["therapeutic_category"]:
            for tc in row["therapeutic_category"].split(", "):
                tc_rows.append(tc.strip())
    if not tc_rows:
        st.info("Therapeutic category data is not available from the current FDA shortage feed.")
    else:
        tc_counts = pd.Series(tc_rows).value_counts().head(10).reset_index()
        tc_counts.columns = ["Therapeutic Category", "Count"]
        tc_counts["% of Other"] = (tc_counts["Count"] / other_count * 100).round(1).astype(str) + "%"
        df_show(tc_counts, height=400)

    # ── Full records table ────────────────────────────────────────────────────
    section("All 'Other' Records")
    display_cols = {
        ...
        "mfr_reason":           "Mfr-Stated Reason",
        "initial_posting_date": "Posted",
        "update_date":          "Updated",
    }
    display_df = edf[list(display_cols.keys())].rename(columns=display_cols)
    ...
    with fc1:
        status_filter = st.multiselect(
            "Filter by Status",
            sorted(display_df["Status"].unique().tolist()),
            default=sorted(display_df["Status"].unique().tolist()),
        )
    with fc2:
        avail_filter = st.multiselect(
            "Filter by Availability",
            sorted(display_df["Availability"].replace("", "Not Reported").unique().tolist()),
            default=sorted(display_df["Availability"].replace("", "Not Reported").unique().tolist()),
        )
    ...
    if filtered_df.empty:
        ...
        df_show(filtered_df, height=400)
        st.caption(f"Showing {len(filtered_df):,} of {other_count:,} 'Other' records")
    ...
    st.download_button(
        label="Export to CSV",
        data=csv_bytes,
        file_name=f"shortage_other_records_{datetime.utcnow().strftime('%Y%m%d')}.csv",
        mime="text/csv",
